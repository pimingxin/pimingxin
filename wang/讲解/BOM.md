...
* 什么是BOM？
  * Browser Object Model                          //通过一些对象操作浏览器
* DOM一般指document上提供的相关API              
  * API Application Programming Interface 应用编程接口  //编程
    * 以函数，类，方法，属性等体现出来
  * GUI 图形用户界面                                    //鼠标键盘
    * 以画面和键鼠操作的形式体现
  * CLI 命令行用户界面Command Line Interface            //命令行
    * 以命令的输入与输出为体现
...
* 这几个对象可以分别为我们提供与页面和浏览器相关的各种信息
  ...
  * location
    ...
    * hostname 域名-不带端口号
    * hash URL#后面一部分
      URL:https://user:pass@www.xxx.com:8899/foo/bar.html?aa=bb&cc=dd#baz
      https 协议
      user 用户名
      pass 密码
      www.xxx.com hostname域名不带端口号
      :8899 端口
      /foo/bar path路径
      ?aa=bb&cc=dd查询
      #bar hash
